 ...
 * Bookmarks Page
 *
 * Displays a list of bookmarked posts. This is a simple placeholder
 * implementation that will later query the `post_bookmarks` table and
 * render the corresponding posts. For now, it shows dummy content
 * demonstrating the layout and styling consistent with the rest of
 * the application.
 */
    ...
    <div className="flex h-screen bg-[#0a0a0c] text-gray-100 font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#1e1f22] md:my-2 md:mr-2 md:rounded-[40px] shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/[0.03] overflow-hidden relative">
        <header className="h-20 flex items-center px-10 bg-[#1e1f22]/80 backdrop-blur-2xl border-b border-white/[0.03]">
          <h1 className="text-3xl font-black uppercase tracking-widest">Bookmarks</h1>
        </header>
        <main className="flex-1 overflow-y-auto p-10 scrollbar-hide space-y-8">
          <div className="max-w-3xl mx-auto">
            <h2 className="text-2xl font-bold mb-4">Gespeicherte Posts</h2>
            <p className="text-gray-400 mb-8">Hier findest du alle Posts, die du als Bookmark markiert hast. Dieses Feature wird im späteren Verlauf mit Supabase‑Daten verknüpft.</p>
            <div className="space-y-6">
              {Array.from({ length: 4 }).map((_, i) => (
                ...
                >
                  ...
                  <p className="text-gray-500 text-sm">Dies ist ein Platzhaltertext für einen Post, der vom Benutzer gespeichert wurde.</p>
                </div>
              ))}
            </div>
          </div>
