  ...
  if (loading) {
    return (
      <div className="h-screen bg-[#1e1f22] flex flex-col items-center justify-center text-white">
        <div className="animate-pulse text-2xl font-black tracking-tighter mb-4 italic uppercase">
          MIGOCHAT
        </div>
        <div className="w-48 h-1 bg-gray-800 rounded-full overflow-hidden">
          <div className="bg-indigo-500 h-full animate-progress shadow-[0_0_10px_rgba(99,102,241,0.5)]" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-[#313338] overflow-hidden font-sans select-none text-rendering-optimizeLegibility">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-12 border-b border-[#1e1f22] flex items-center justify-between px-2 sm:px-4 shadow-md bg-[#313338] shrink-0 z-10">
          <div className="flex items-center gap-2 sm:gap-4 text-gray-400 font-medium text-sm min-w-0">
            {/* ✅ Mobile Burger */}
            <button
              onClick={() => setMobileMenuOpen(true)}
              className="md:hidden text-gray-300 hover:text-white px-2 py-1 rounded hover:bg-[#393c41]"
              aria-label="Menü öffnen"
            >
              ☰
            </button>

            <div className="flex items-center gap-2 text-white border-r border-gray-700 pr-3 sm:pr-4 min-w-0">
              <span className="text-gray-500 text-xl font-light">@</span>
              <span className="font-bold tracking-wide truncate">Freunde</span>
            </div>

            {/* Tabs: auf klein etwas kompakter */}
            <nav className="flex gap-1 overflow-x-auto">
              {[
                ...
                <button
                  ...
                  className={`px-2 sm:px-3 py-1 rounded-md transition-all duration-200 font-semibold whitespace-nowrap ${
                    activeTab === tab.id
          ...
          <button
            ...
            className="bg-[#248046] hover:bg-[#1a6334] active:bg-[#14522b] text-white text-xs font-bold py-1.5 px-3 rounded shadow-lg transition-all active:scale-95 whitespace-nowrap"
          >
            Freund hinzufügen
          </button>
        </header>
        ...
        <main className="flex-1 flex overflow-hidden">
          ...
          <aside className="w-80 border-l border-gray-800 hidden xl:flex flex-col p-4 bg-[#313338]">
            <h3 className="text-white font-black mb-4 tracking-wider text-[11px] uppercase opacity-70">
              Aktivität
            </h3>
